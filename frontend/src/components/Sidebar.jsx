    ...
    if (collapsed) {
        return (
            <div className='hidden lg:flex flex-col items-center w-14 h-screen bg-[#0d0f14] border-r border-white/6 py-4 gap-1 shrink-0'>
                ...
                <button className='flex items-center justify-center w-9 h-9 rounded-xl text-slate-500 hover:text-slate-200 hover:bg-white/5 transition-colors duration-150 bg-transparent border-none cursor-pointer' onClick={() => dispatch(setSelectedConversation(null))}>
                    <Plus size={17} />
                </button>
                ...
                <div className='flex-1 overflow-y-auto px-2.5 pb-2 scrollbar-none [&::-webkit-scrollbar]:hidden pt-5'>
                    {
                        conversations.map((conv, i) => {
                            ...
                            return (
                                ...
                                >
                                    <div className={`flex items-center justify-center shrink-0 w-6 h-6 rounded-lg transition-colors duration-150 ${isActive ? "bg-indigo-500/15 text-indigo-400" : "bg-white/5 text-slate-500"}`}>
        ...
        <div className='fixed lg:static inset-y-0 left-0 z-50 w-67.5 h-screen shrink-0 bg-[#0d0f14] border-r border-white/6'>
            <div className='flex flex-col h-full'>
                ...
                <div className='flex items-center gap-2.5 px-4 py-4 border-b border-white/6'>
                    ...
                    <span className='text-[16px] font-semibold text-slate-100 tracking-tight flex-1'>CoreTex AI</span>
                    ...
                    <button className='flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-white/5 transition-colors duration-150 bg-transparent border-none cursor-pointer' onClick={() => dispatch(setSelectedConversation(null))}>
                        <PenSquare size={14} />
                    </button>
                </div>
                ...
                <div className='px-4 pt-4 pb-1'>
                    <button className='w-full flex items-center justify-center gap-2 text-sm font-medium text-white bg-linear-to-br from-indigo-500 to-violet-700 rounded-xl py-2.5 border-none cursor-pointer hover:opacity-90 transition-opacity duration-150' onClick={() => dispatch(setSelectedConversation(null))}>
                        ...
                        conversations.map((conv, i) => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`flex items-center gap-2.5 cursor-pointer mb-0.5 px-3 py-2.5 rounded-[10px] border transition-colors duration-150 ${isActive ? "bg-indigo-500/10 border-indigo-500/18" : "bg-transparent border-transparent"}`}
                                >
                                    <div className={`flex items-center justify-center shrink-0 w-7 h-7 rounded-lg transition-colors duration-150 ${isActive ? "bg-indigo-500/15 text-indigo-400" : "bg-white/5 text-slate-500"}`}>
                                        <MessageSquare size={13} />
                                    </div>
